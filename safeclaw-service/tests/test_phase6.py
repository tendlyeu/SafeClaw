...
from safeclaw.engine.multi_agent import MultiAgentGovernor, AgentContext, MAX_AGENTS
...
# --- MultiAgentGovernor Tests ---

class TestMultiAgentGovernor:
    def test_register_and_get_agent(self):
        """register_agent and get_agent."""
        gov = MultiAgentGovernor()
        ctx = gov.register_agent("agent-1", session_id="s1")
        assert ctx.agent_id == "agent-1"
        assert ctx.session_id == "s1"
        assert ctx.parent_id is None

        retrieved = gov.get_agent("agent-1")
        assert retrieved is not None
        assert retrieved.agent_id == "agent-1"

    def test_register_with_parent_id(self):
        """register_agent with parent_id links parent and child."""
        gov = MultiAgentGovernor()
        gov.register_agent("root", session_id="s1")
        child = gov.register_agent("child-1", session_id="s1", parent_id="root")
        assert child.parent_id == "root"

    def test_get_children_returns_direct_only(self):
        """get_children returns only direct children, not grandchildren."""
        gov = MultiAgentGovernor()
        gov.register_agent("root", session_id="s1")
        gov.register_agent("child-1", session_id="s1", parent_id="root")
        gov.register_agent("child-2", session_id="s1", parent_id="root")
        gov.register_agent("grandchild-1", session_id="s1", parent_id="child-1")

        children = gov.get_children("root")
        child_ids = [c.agent_id for c in children]
        assert "child-1" in child_ids
        assert "child-2" in child_ids
        assert "grandchild-1" not in child_ids
        assert len(children) == 2

    def test_get_ancestry_returns_root_to_leaf(self):
        """get_ancestry returns the chain from root to the given agent."""
        gov = MultiAgentGovernor()
        gov.register_agent("root", session_id="s1")
        gov.register_agent("mid", session_id="s1", parent_id="root")
        gov.register_agent("leaf", session_id="s1", parent_id="mid")

        ancestry = gov.get_ancestry("leaf")
        assert ancestry == ["root", "mid", "leaf"]

    def test_set_and_get_constraint_override(self):
        """set_constraint_override and retrieve via agent context."""
        gov = MultiAgentGovernor()
        gov.register_agent("agent-1", session_id="s1")
        gov.set_constraint_override("agent-1", "max_risk", "MediumRisk")

        ctx = gov.get_agent("agent-1")
        assert ctx.constraint_overrides["max_risk"] == "MediumRisk"

    def test_get_effective_constraints_merges_parent_child(self):
        """get_effective_constraints merges constraints from parent to child."""
        gov = MultiAgentGovernor()
        gov.register_agent("root", session_id="s1")
        gov.set_constraint_override("root", "max_risk", "HighRisk")
        gov.set_constraint_override("root", "allow_network", "true")

        gov.register_agent("child", session_id="s1", parent_id="root")
        gov.set_constraint_override("child", "max_risk", "MediumRisk")  # tighter override

        effective = gov.get_effective_constraints("child")
        # Child's override should win over parent for max_risk
        assert effective["max_risk"] == "MediumRisk"
        # Parent's allow_network should be inherited
        assert effective["allow_network"] == "true"

    def test_unregister_agent_removes_it(self):
        """unregister_agent removes the agent from governance."""
        gov = MultiAgentGovernor()
        gov.register_agent("agent-1", session_id="s1")
        assert gov.get_agent("agent-1") is not None

        gov.unregister_agent("agent-1")
        assert gov.get_agent("agent-1") is None

    def test_max_agents_eviction(self, monkeypatch):
        """When MAX_AGENTS is exceeded, oldest agents are removed."""
        import safeclaw.engine.multi_agent as ma_mod
        monkeypatch.setattr(ma_mod, "MAX_AGENTS", 10)

        gov = MultiAgentGovernor()
        for i in range(15):
            gov.register_agent(f"agent-{i}", session_id="s1")

        # Oldest agents should have been evicted
        assert gov.get_agent("agent-0") is None
        assert gov.get_agent("agent-4") is None
        # Newest should still exist
        assert gov.get_agent("agent-14") is not None

    def test_get_agent_nonexistent_returns_none(self):
        """get_agent for nonexistent ID returns None."""
        gov = MultiAgentGovernor()
        assert gov.get_agent("nonexistent") is None

    def test_unregister_nonexistent_no_error(self):
        """unregister_agent for nonexistent ID does not raise."""
        gov = MultiAgentGovernor()
        gov.unregister_agent("nonexistent")  # should not raise
