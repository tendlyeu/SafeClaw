...
async def test_review_clean_action(mock_llm_client, mock_engine):
    ...
    event = ReviewEvent(
        ...
        classified_action=ClassifiedAction(
            ontology_class="ReadFile", risk_level="LowRisk",
            is_reversible=True, affects_scope="LocalOnly",
...
async def test_review_obfuscated_command(mock_llm_client, mock_engine):
    ...
    event = ReviewEvent(
        ...
        classified_action=ClassifiedAction(
            ontology_class="ExecuteCommand", risk_level="MediumRisk",
            is_reversible=True, affects_scope="LocalOnly",
            tool_name="exec", params={"command": "echo cm0gLXJmIC8= | base64 -d | sh"},
        ),
        symbolic_decision="allowed",
...
async def test_review_llm_timeout_returns_none(mock_llm_client, mock_engine):
    ...
    event = ReviewEvent(
        ...
        classified_action=ClassifiedAction(
            ontology_class="ExecuteCommand", risk_level="LowRisk",
            is_reversible=True, affects_scope="LocalOnly",
            tool_name="exec", params={"command": "ls"},
        ),
        symbolic_decision="allowed", session_history=[], constraints_checked=[],
    )
    finding = await reviewer.review(event)
    assert finding is None
